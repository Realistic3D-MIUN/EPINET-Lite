...
from tensorflow.keras.layers import Activation, LeakyReLU, PReLU, Lambda
...
GLOBAL_ATTENTION_TYPE = 'se'  # choose from ['se', 'eca', 'cbam', 'coord', 'sqex']
...
def get_activation(name='relu', layer_name='', **kwargs):
    ...
    if name == 'relu':
        ...
        return Activation(lambda x: tf.clip_by_value(x, -3.0, x), name=layer_name, **kwargs)
    elif name == 'leakyrelu': # [0.01, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3]
        return LeakyReLU(alpha=0.3, name=layer_name, **kwargs)
    elif name == 'prelu':
        return PReLU(name=layer_name, **kwargs)
    elif name == 'tanh':
        return Activation('tanh', name=layer_name, **kwargs)
    elif name == 'scaled_tanh':
        return Lambda(lambda x: 3.0 * tf.tanh(x), name=layer_name, **kwargs)
    elif name == 'softsign':
        return Activation('softsign', name=layer_name, **kwargs)
    elif name == 'hardtanh':
        return Lambda(lambda x: tf.clip_by_value(x, -1.0, 1.0), name=layer_name, **kwargs)
    elif name == 'identity':
        return Lambda(lambda x: x, name=layer_name, **kwargs)
    else:
        raise ValueError(f"Unsupported activation function: {name}")


def get_k_size2(kernel_size):
    k = [2]
    if kernel_size == 1:
        k = [2]
    elif kernel_size == 2:
...
def layer1_multistream(input_dim1, input_dim2, input_dim3, filt_num, kernel_size, msname):
    global conv_type, ACTIVATION_FN, GLOBAL_ATTENTION_TYPE
...
def layer3_last(input_dim1, input_dim2, input_dim3, filt_num, kernel_size):
    ...
    #x = Conv2D(128, (3, 3), padding=paddingType, use_bias=False, name='S3_128_last')(x)
    #x = get_activation(ACTIVATION_FN, layer_name='S3_128_act')(x)
    #x = Conv2D(64, (3, 3), padding=paddingType, use_bias=False, name='S3_64_last')(x)
    #x = get_activation(ACTIVATION_FN, layer_name='S3_64_act')(x)
    #x = Conv2D(32, (3, 3), padding=paddingType, use_bias=False, name='S3_32_last')(x)
    #x = get_activation(ACTIVATION_FN, layer_name='S3_32_act')(x)
...
def define_epinet(sz_input, sz_input2, view_n, conv_depth, filt_num, learning_rate, kernel_size):
    ...
    model_512.compile(optimizer=opt, loss='mae', metrics=[metrics.MeanSquaredError(), bpr]) #org
    return model_512
