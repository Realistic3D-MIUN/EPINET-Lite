...
import numpy as np
import imageio
from epinet_func.evaluation_metric import PSNR
...
def measurePerformance(model_output, valdata_label, networkName, LFname):
    ...
    if networkName != None:
        model_output_all = np.zeros((2 * 512, sz * 512), np.uint8)
        model_output_all[0:512, :] = np.uint8(
            25 * np.reshape(np.transpose(valdata_label), (512, sz * 512)) + 100)
        model_output_all[512:2 * 512, :] = np.uint8(
            25 * np.reshape(np.transpose(model_output), (512, sz * 512)) + 100)
        ...
        imageio.imsave(fName, np.squeeze(model_output_all))

    train_diff = np.abs(model_output - valdata_label)
    train_bp7 = (train_diff >= 0.07)
    train_bp3 = (train_diff >= 0.03)
    train_bp1 = (train_diff >= 0.01)
    train_psnr = PSNR(valdata_label, model_output)
