...
#    seq.add(Reshape((input_dim1,input_dim12,input_dim3),input_shape=(input_dim1, input_dim2, input_dim3,1)))
    for i in range(3):
        seq.add(Conv2D(int(filt_num),(kernel_size,kernel_size),input_shape=(input_dim1, input_dim2, input_dim3), padding=padding, name='S1_c1%d' %(i) ))
        seq.add(Activation('relu', name='S1_relu1%d' %(i))) 
        seq.add(Conv2D(int(filt_num),(kernel_size,kernel_size), padding=padding, name='S1_c2%d' %(i) )) 
...
def layer2_merged(input_dim1,input_dim2,input_dim3,filt_num,conv_depth,kernel_size):
    ...
    for i in range(conv_depth):
        seq.add(Conv2D(filt_num,(kernel_size,kernel_size), padding=padding,input_shape=(input_dim1, input_dim2, input_dim3), name='S2_c1%d' % (i) ))
        seq.add(Activation('relu', name='S2_relu1%d' %(i))) 
        seq.add(Conv2D(filt_num,(kernel_size,kernel_size), padding=padding, name='S2_c2%d' % (i))) 
...
def layer3_last(input_dim1,input_dim2,input_dim3,filt_num,kernel_size):   
    ...
    for i in range(1):
        seq.add(Conv2D(filt_num,(kernel_size,kernel_size), padding=padding,input_shape=(input_dim1, input_dim2, input_dim3), name='S3_c1%d' %(i) )) # pow(25/23,2)*12*(maybe7?) 43 3
...
def define_epinet(sz_input,sz_input2,view_n,conv_depth,filt_num,learning_rate,kernel_size):
    ...
    ''' Merged layer : Conv - Relu - Conv - BN - Relu '''
    #mid_merged_=layer2_merged(sz_input-6,sz_input2-6,int(4*filt_num),int(4*filt_num),conv_depth,kernel_size)(mid_merged)
    ...
    #output=layer3_last(sz_input-20,sz_input2-20,int(4*filt_num),int(4*filt_num),kernel_size)(mid_merged_)
