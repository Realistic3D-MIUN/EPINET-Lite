...
from epinet_fun.component import ghostmodule
#from epinet_fun.component import GhostModule
import keras_flops
...
def layer1_multistream_ghost(input_dim1,input_dim2,input_dim3,filt_num):
    seq = Sequential()
    seq.add(Input(shape=(input_dim1, input_dim2, input_dim3)))
...
#    seq.add(Reshape((input_dim1,input_dim12,input_dim3),input_shape=(input_dim1, input_dim2, input_dim3,1)))
    for i in range(3):
        seq.add(ghostmodule.GhostModule((seq.output_shape[1],seq.output_shape[2],seq.output_shape[3]),filt_num, ratio, convKernel, dwKernel))
    ...
    if(printStat):
        ...
        vList.append(values)
        print(profile)
        seq.summary(expand_nested=True)
    return seq

def layer2_merged_ghost(input_dim1,input_dim2,input_dim3,filt_num,conv_depth):
...
def layer3_last_ghost(input_dim1,input_dim2,input_dim3,filt_num):
...
def define_epinet_ghost(sz_input,sz_input2,view_n,conv_depth,filt_num,learning_rate):
    ...
    mid_90d=layer1_multistream_ghost(sz_input,sz_input2,len(view_n),int(filt_num))(input_stack_90d)
    mid_0d=layer1_multistream_ghost(sz_input,sz_input2,len(view_n),int(filt_num))(input_stack_0d)
    mid_45d=layer1_multistream_ghost(sz_input,sz_input2,len(view_n),int(filt_num))(input_stack_45d)
    mid_M45d=layer1_multistream_ghost(sz_input,sz_input2,len(view_n),int(filt_num))(input_stack_M45d)

    ''' Merge layers ''' 
    mid_merged = concatenate([mid_90d,mid_0d,mid_45d,mid_M45d])

    ''' Merged layer : Conv - Relu - Conv - BN - Relu '''
    mid_merged_=layer2_merged_ghost(sz_input-6,sz_input2-6,int(4*filt_num),int(4*filt_num),conv_depth)(mid_merged)

    ''' Last Conv layer : Conv - Relu - Conv '''
    output=layer3_last_ghost(sz_input-20,sz_input2-20,int(4*filt_num),int(4*filt_num))(mid_merged_)
...
def evaluateEPINet_ghost():
    ...
    model = define_epinet_ghost(image_w, image_h,
