...
def layer1_multistream_sc(input_dim1, input_dim2, input_dim3, filt_num):
    seq = Sequential()
    ...
    for i in range(3):
        seq.add(SeparableConv2D(int(filt_num), (2, 2), input_shape=(input_dim1, input_dim2, input_dim3),
                                padding=paddingType, name='conv_dw_%d' % (i)))
        seq.add(Activation('relu', name='S1_relu1%d' % (i)))
        seq.add(Conv2D(int(filt_num), (2, 2), padding=paddingType, name='S1_c2%d' % (i)))
...
def layer2_merged_sc(input_dim1, input_dim2, input_dim3, filt_num, conv_depth):
    ...
    for i in range(conv_depth):
        ...
        seq.add(Conv2D(filt_num, (2, 2), padding=paddingType, name='S2_c2%d' % (i)))
...
def layer3_last_sc(input_dim1, input_dim2, input_dim3, filt_num):
    ...
    seq.add(Conv2D(1, (2, 2), padding=paddingType, use_bias=False, name='S3_last'))
    if (printStat):
        # flops = evaluation_metric.net_flops(seq, table=True)
        # flops2 = keras_flops.get_flops(seq,batch_size=1)
        # print('FLOPS2: ',flops2)
...
def define_epinet(sz_input, sz_input2, view_n, conv_depth, filt_num, learning_rate, kernel_size):
    ...
    mid_90d = layer1_multistream_sc(sz_input, sz_input2, len(view_n), int(filt_num))(input_stack_90d)
    mid_0d = layer1_multistream_sc(sz_input, sz_input2, len(view_n), int(filt_num))(input_stack_0d)
    mid_45d = layer1_multistream_sc(sz_input, sz_input2, len(view_n), int(filt_num))(input_stack_45d)
    mid_M45d = layer1_multistream_sc(sz_input, sz_input2, len(view_n), int(filt_num))(input_stack_M45d)

    ''' Merge layers '''
    mid_merged = concatenate([mid_90d, mid_0d, mid_45d, mid_M45d])

    ''' Merged layer : Conv - Relu - Conv - BN - Relu '''
    #mid_merged_ = layer2_merged_sc(sz_input - 6, sz_input2 - 6, int(4 * filt_num), int(4 * filt_num), conv_depth)(
    #    mid_merged)
    mid_merged_ = layer2_merged_sc(sz_input, sz_input2, int(4 * filt_num), int(4 * filt_num), conv_depth)(
        mid_merged)

    ''' Last Conv layer : Conv - Relu - Conv '''
    #output = layer3_last_sc(sz_input - 20, sz_input2 - 20, int(4 * filt_num), int(4 * filt_num))(mid_merged_)
    output = layer3_last_sc(sz_input, sz_input2, int(4 * filt_num), int(4 * filt_num))(mid_merged_)
...
def evaluateEPINet_sc():
    ...
    model = define_epinet_sc(image_w, image_h,
